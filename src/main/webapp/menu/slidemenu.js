...
createSlideMenu.prototype.makeMenu = function(type,text,lnk,target,end){
  ...
  if(m.lev>=this.l.length){//Level control (same as CM4)
	...
	if(!this.writesubsonly || type!="top"){
		...
		str+='<div class="'+this.txtClass+'"><a href="'+lnk+'" '+tg+' OnMouseOver="window.status=\''+text+'- IOTECA 2.0\';return true"'
		str+=' onfocus="window.status=\''+text+'- IOTECA 2.0\';return true" '
        str+=' onclick="'+this.name+'.switchMenu('+this.curr+'); if(document.getElementById) this.blur(); '
